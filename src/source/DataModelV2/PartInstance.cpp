...
PartInstance::PartInstance(void)
{
    ...
    OnTouchAction = Enum::ActionType::Nothing;
    OnTouchSound = Enum::Sound::NoSound;
...
void PartInstance::setOnTouchAction(Enum::ActionType::Value value)
{
    OnTouchAction = value;
}

void PartInstance::setOnTouchSound(Enum::Sound::Value value)
{
    OnTouchSound = value;
}

void PartInstance::setChangeScore(int value)
{
    changeScore = value;
}

void PartInstance::setChangeTimer(float value)
{
    changeTimer = value;
}

void PartInstance::setSingleShot(bool value)
{
    singleShot = value;
}
...
{
    setSurface(face, surface, 0);
}

void PartInstance::setSurface(int face, Enum::SurfaceType::Value surface, int motorParam)
{
	switch(face)
	{
	case TOP:
	...
	if (face >= 0 && face < 6)
		surfaceParam[face] = motorParam;
	changed = true;
}

Enum::SurfaceType::Value PartInstance::getSurface(int face) const
{
	switch (face)
	{
	case TOP: return top;
...
int PartInstance::getSurfaceParam(int face) const
{
	if (face < 0 || face >= 6)
		return 0;
	return surfaceParam[face];
}
...
PartInstance::PartInstance(const PartInstance &oinst)
{
	...
	bottom = oinst.bottom;
	for (int i = 0; i < 6; ++i)
		surfaceParam[i] = oinst.surfaceParam[i];
	shape = oinst.shape;
	nameShown = oinst.nameShown;
	controllerFlagShown = oinst.controllerFlagShown;
	controller = oinst.controller;
	changed = true;
...
Sphere PartInstance::getSphere()
{
	...
	if (size.z > radius)
		radius = size.z;
	return Sphere(c.translation, radius / 2.0f);
}
...
void PartInstance::PropUpdate(LPPROPGRIDITEM &item)
{
	...
	if(strcmp(item->lpszPropName, "Color3") == 0)
	{
		unsigned long colorValue =
			(unsigned long)item->lpCurValue;
		int red = (int)(colorValue & 0xFF);
		int green = (int)((colorValue >> 8) & 0xFF);
		int blue = (int)((colorValue >> 16) & 0xFF);
		color = Color3(
			red / 255.0F,
			green / 255.0F,
			blue / 255.0F
