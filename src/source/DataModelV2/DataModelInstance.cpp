...
static const char* ShapeToString(Enum::Shape::Value shape);
static const char* SurfaceToString(Enum::SurfaceType::Value surface);
static Enum::SurfaceType::Value SurfaceFromString(const char* value);
static const char* ControllerToString(Enum::Controller::Value controller);
static Enum::Controller::Value ControllerFromString(const char* value);
	...
	for (xml_node<> *node = scanNode->first_node();node; node = node->next_sibling())
	{
		...
		if (strncmp(node->name(),"Item",4)==0)
		{
			...
			if (className=="Part") {
				...
				std::string controllerValue;
				std::string topSurfaceValue;
				std::string frontSurfaceValue;
				std::string rightSurfaceValue;
				std::string backSurfaceValue;
				std::string leftSurfaceValue;
				std::string bottomSurfaceValue;
				int topSurfaceParam = 0;
				int frontSurfaceParam = 0;
				int rightSurfaceParam = 0;
				int backSurfaceParam = 0;
				int leftSurfaceParam = 0;
				int bottomSurfaceParam = 0;
				for (xml_node<> *surfaceNode = propNode->first_node(); surfaceNode; surfaceNode = surfaceNode->next_sibling())
				{
					xml_attribute<> *nameAttr = surfaceNode->first_attribute("name");
					if (nameAttr == NULL)
						continue;
					std::string propName = nameAttr->value();
					if (propName == "Controller")
						controllerValue = surfaceNode->value();
					else if (propName == "TopSurface")
						topSurfaceValue = surfaceNode->value();
					else if (propName == "FrontSurface")
						frontSurfaceValue = surfaceNode->value();
					else if (propName == "RightSurface")
						rightSurfaceValue = surfaceNode->value();
					else if (propName == "BackSurface")
						backSurfaceValue = surfaceNode->value();
					else if (propName == "LeftSurface")
						leftSurfaceValue = surfaceNode->value();
					else if (propName == "BottomSurface")
						bottomSurfaceValue = surfaceNode->value();
					else if (propName == "TopSurfaceParam")
						topSurfaceParam = atoi(surfaceNode->value());
					else if (propName == "FrontSurfaceParam")
						frontSurfaceParam = atoi(surfaceNode->value());
					else if (propName == "RightSurfaceParam")
						rightSurfaceParam = atoi(surfaceNode->value());
					else if (propName == "BackSurfaceParam")
						backSurfaceParam = atoi(surfaceNode->value());
					else if (propName == "LeftSurfaceParam")
						leftSurfaceParam = atoi(surfaceNode->value());
					else if (propName == "BottomSurfaceParam")
						bottomSurfaceParam = atoi(surfaceNode->value());
				}
				...
				if (_successfulLoad) {
					...
					if (!controllerValue.empty())
						test->controller = ControllerFromString(controllerValue.c_str());
					if (!topSurfaceValue.empty())
						test->setSurface(TOP, SurfaceFromString(topSurfaceValue.c_str()), topSurfaceParam);
					if (!frontSurfaceValue.empty())
						test->setSurface(FRONT, SurfaceFromString(frontSurfaceValue.c_str()), frontSurfaceParam);
					if (!rightSurfaceValue.empty())
						test->setSurface(RIGHT, SurfaceFromString(rightSurfaceValue.c_str()), rightSurfaceParam);
					if (!backSurfaceValue.empty())
						test->setSurface(BACK, SurfaceFromString(backSurfaceValue.c_str()), backSurfaceParam);
					if (!leftSurfaceValue.empty())
						test->setSurface(LEFT, SurfaceFromString(leftSurfaceValue.c_str()), leftSurfaceParam);
					if (!bottomSurfaceValue.empty())
						test->setSurface(BOTTOM, SurfaceFromString(bottomSurfaceValue.c_str()), bottomSurfaceParam);
				}
				else
				{
					return false;
...
static const char* ShapeToString(Enum::Shape::Value shape)
{
    switch (shape)
    {
    case Enum::Shape::Ball: return "0";
    case Enum::Shape::Block: return "1";
    case Enum::Shape::Cylinder: return "2";
    default: return "1";
    }
}
...
static const char* SurfaceToString(Enum::SurfaceType::Value surface)
{
    switch (surface)
    {
    case Enum::SurfaceType::Smooth: return "Smooth";
    case Enum::SurfaceType::Bumps: return "Bumps";
    case Enum::SurfaceType::Hinge: return "Hinge";
    case Enum::SurfaceType::Motor: return "Motor";
    case Enum::SurfaceType::StepperMotor: return "StepperMotor";
    case Enum::SurfaceType::Spawn: return "Spawn";
...
static Enum::SurfaceType::Value SurfaceFromString(const char* value)
{
    ...
    std::string surface(value);
    if (surface == "Bumps" || surface == "1") return Enum::SurfaceType::Bumps;
    if (surface == "Hinge" || surface == "2") return Enum::SurfaceType::Hinge;
    if (surface == "Motor" || surface == "3") return Enum::SurfaceType::Motor;
    if (surface == "StepperMotor" || surface == "4") return Enum::SurfaceType::StepperMotor;
    if (surface == "Spawn" || surface == "5") return Enum::SurfaceType::Spawn;
    return Enum::SurfaceType::Smooth;
}
...
static const char* ControllerToString(Enum::Controller::Value controller)
{
    switch (controller)
    {
    case Enum::Controller::None: return "None";
    case Enum::Controller::KeyboardRight: return "KeyboardRight";
    case Enum::Controller::KeyboardLeft: return "KeyboardLeft";
    case Enum::Controller::Joypad1: return "Joypad1";
    case Enum::Controller::Joypad2: return "Joypad2";
    case Enum::Controller::Chase: return "Chase";
    case Enum::Controller::Flee: return "Flee";
    default: return "None";
    }
}
...
static Enum::Controller::Value ControllerFromString(const char* value)
{
    ...
    if (controller == "KeyboardRight") return Enum::Controller::KeyboardRight;
    if (controller == "KeyboardLeft") return Enum::Controller::KeyboardLeft;
    if (controller == "Joypad1") return Enum::Controller::Joypad1;
    if (controller == "Joypad2") return Enum::Controller::Joypad2;
    if (controller == "Chase") return Enum::Controller::Chase;
    if (controller == "Flee") return Enum::Controller::Flee;
    return Enum::Controller::None;
}

bool DataModelInstance::save()
{
    if (_loadedFileName.empty())
        return saveAs();
    return saveToFile(_loadedFileName.c_str());
}

bool DataModelInstance::saveAs()
{
    OPENFILENAME of;
    char szFile[512];
    ZeroMemory(&of, sizeof(of));
    ...
    of.lStructSize = sizeof(OPENFILENAME);
    of.lpstrFilter = "Blocks3D Files\0*.rbxm;*.rbxl;*.b3d\0All Files\0*.*\0\0";
    of.lpstrFile = szFile;
    of.nMaxFile = sizeof(szFile);
    of.lpstrTitle = "Save Blocks3D Level";
...
bool DataModelInstance::saveToFile(const char* filename)
{
    std::ofstream file(filename, std::ios::out | std::ios::trunc);
    if (!file)
    {
        std::stringstream message;
        message << "Failed to save file:" << std::endl << filename
                << std::endl << strerror(errno);
        MessageBoxStr(message.str());
        return false;
    }

    file << "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
    file << "<roblox version=\"4\">\n";
    file << "  <Item class=\"Workspace\" referent=\"RBXWORKSPACE\">\n";
    file << "    <Properties>\n"
            "      <string name=\"Name\">Workspace</string>\n"
            "    </Properties>\n";

    for (size_t i = 0; i < workspace->partObjects.size(); ++i)
    {
        PartInstance* part = workspace->partObjects[i];
        if (part == NULL)
            continue;
        ...
        file << "    <Item class=\"Part\">\n"
                "      <Properties>\n";
        file << "        <string name=\"Name\">" << XmlEscape(part->name)
             << "</string>\n";
        file << "        <CoordinateFrame name=\"CFrame\">\n"
                "          <X>" << std::setprecision(9) << position.x << "</X>\n"
                "          <Y>" << position.y << "</Y>\n"
                "          <Z>" << position.z << "</Z>\n"
                "          <R00>" << rotation[0][0] << "</R00>\n"
                "          <R01>" << rotation[0][1] << "</R01>\n"
                "          <R02>" << rotation[0][2] << "</R02>\n"
                "          <R10>" << rotation[1][0] << "</R10>\n"
                "          <R11>" << rotation[1][1] << "</R11>\n"
                "          <R12>" << rotation[1][2] << "</R12>\n"
                "          <R20>" << rotation[2][0] << "</R20>\n"
                "          <R21>" << rotation[2][1] << "</R21>\n"
                "          <R22>" << rotation[2][2] << "</R22>\n"
                "        </CoordinateFrame>\n";
        file << "        <bool name=\"Anchored\">"
             << (part->isAnchored() ? "true" : "false")
             << "</bool>\n";
        file << "        <token name=\"shape\">" << ShapeToString(part->shape)
             << "</token>\n";
        file << "        <token name=\"Controller\">" << ControllerToString(part->controller) << "</token>\n";
        file << "        <token name=\"TopSurface\">" << SurfaceToString(part->top) << "</token>\n";
        file << "        <token name=\"FrontSurface\">" << SurfaceToString(part->front) << "</token>\n";
        file << "        <token name=\"RightSurface\">" << SurfaceToString(part->right) << "</token>\n";
        file << "        <token name=\"BackSurface\">" << SurfaceToString(part->back) << "</token>\n";
        file << "        <token name=\"LeftSurface\">" << SurfaceToString(part->left) << "</token>\n";
        file << "        <token name=\"BottomSurface\">" << SurfaceToString(part->bottom) << "</token>\n";
        file << "        <token name=\"TopSurfaceParam\">" << part->getSurfaceParam(TOP) << "</token>\n";
        file << "        <token name=\"FrontSurfaceParam\">" << part->getSurfaceParam(FRONT) << "</token>\n";
        file << "        <token name=\"RightSurfaceParam\">" << part->getSurfaceParam(RIGHT) << "</token>\n";
        file << "        <token name=\"BackSurfaceParam\">" << part->getSurfaceParam(BACK) << "</token>\n";
        file << "        <token name=\"LeftSurfaceParam\">" << part->getSurfaceParam(LEFT) << "</token>\n";
        file << "        <token name=\"BottomSurfaceParam\">" << part->getSurfaceParam(BOTTOM) << "</token>\n";
        file << "        <Color3 name=\"Color\">\n"
                "          <R>" << part->color.r << "</R>\n"
                "          <G>" << part->color.g << "</G>\n"
                "          <B>" << part->color.b << "</B>\n"
                "        </Color3>\n";
        file << "        <Vector3 name=\"size\">\n"
                "          <X>" << size.x << "</X>\n"
                "          <Y>" << size.y << "</Y>\n"
                "          <Z>" << size.z << "</Z>\n"
                "        </Vector3>\n"
    ...
    if (!file)
    {
        MessageBoxStr("An error occurred while writing the level file.");
        return false;
    }
