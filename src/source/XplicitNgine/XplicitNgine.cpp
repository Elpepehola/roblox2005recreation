...
void XplicitNgine::clearPhysGeom(PartInstance* partInstance)
{
	for (int i = 0; i < 3; ++i)
	{
		if (partInstance->physGeom[i] != NULL)
		{
			dGeomDestroy(partInstance->physGeom[i]);
			partInstance->physGeom[i] = NULL;
		}
	}
}
...
void collisionCallback(void *data, dGeomID o1, dGeomID o2) 
{
	...
	// Do not skip connected bodies.  Stud/fixed joints keep parts
	// touching but the game still needs OnTouch to fire so that
	// Win/Lose/score/timer logic can run.
	...
	if (n > 0) {
		for (i=0; i<n; i++) {
			// Parts are rigid building blocks, not rubber.  Bounce combined
			// with the old fixed timestep caused stacks to gain energy at
			// every contact.  Keep soft contacts for penetration correction.
			...
			// Fire OnTouch for both sides when possible.  If one body
			// is NULL (anchored part), still notify the non-NULL side.
			if(b1 != NULL) 
			{
			...
			if(b2 != NULL) 
			{
				PartInstance* touched = (PartInstance*)dGeomGetData(o1);
				if(touched != NULL) 
				{
					touched->onTouch();
				}
...
void XplicitNgine::deleteBody(PartInstance* partInstance)
{
	if(partInstance->physBody != NULL)
	{
		...
		if(partInstance->physGeom[0] != NULL)
		{
			// Editing can happen while the simulation is paused.  Never
			// advance ODE while tearing down a body; doing so re-enters
			// collision/joint creation during a drag and can invalidate
			// the body or one of its contact joints.
			dGeomSetBody(partInstance->physGeom[0], 0);
