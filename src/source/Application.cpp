...
static bool controlForward = false;
static bool controlBackward = false;
static bool controlLeft = false;
static bool controlRight = false;
...
void Application::onSimulation(RealTime rdt, SimTime sdt, SimTime idt) {
	...
	if(_dataModel->isRunning())
	{
		...
		// Keep the four substeps for contact stability, but divide the
		// simulation time between them.  Passing 0.03 to every substep
		// advanced physics four times faster than the rest of the game.
		float physicsStep = (float)sdt / 4.0F;
		for(int i = 0; i < 4; i++)
		{
			_dataModel->getEngine()->step(physicsStep);
		}
		onLogic();
...
void Application::onKeyPressed(int key)
{
	...
	if(_dataModel != NULL)
		_dataModel->getEngine()->setControlState(
			controlForward,
			controlBackward,
			controlLeft,
			controlRight
